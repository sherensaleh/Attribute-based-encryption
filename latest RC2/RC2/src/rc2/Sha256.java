...
public class Sha256
{
	...
	byte[] nhash=sha.hash(s.getBytes());
        byte[] mhash=sha1.hash(m.getBytes());
    //    mhash+2;
	System.out.println();
	//System.out.println("SHA2 Output(Hashcode) : "+nhash);
      //  String sd="5";
        //System.out.println("Text [Byte Format] : " + nhash.toString());
   int fg=new BigInteger(nhash).intValue();
 // int fg= fg1.intValue();
      int gf=new BigInteger(mhash).intValue();
    //  Byte[] bn=new Byte[](nhash);
   // int fg=  nhash.intValue();
		    String ss = new String(nhash);
                    //int ss=(Integer)sx*2;
                      String sd = new String(mhash);
		    System.out.println("Text Decryted : " + ss);
                     System.out.println("Text  : " + sd);
        System.out.println("byte to int1: "+fg);
            System.out.println("byte to int2: "+gf);
            
//             int result = 0;
//    for (int i=0; i<4; i++) {
//      result = ( result << 8 ) - Byte.MIN_VALUE + (int) nhash[i];
//    }
//          System.out.println("byte to int3: "+fg);
	}
